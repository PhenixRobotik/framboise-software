...
void RP_Init_Interface(RP_Interface *interface,
  ...
  interface->can_id_rx = can_id_rx;
  interface->can_id_tx = can_id_tx;
  interface->send = send;
...
int RP_Sync(RP_Interface *interface, uint32_t timeout){
  ...
  if(interface->send(interface->link_handler, interface->can_id_tx, &byte, 1, timeout) != 0){
...
int RP_Send(RP_Interface *interface, RP_Packet *packet, uint32_t timeout){
  ...
  //printf("Sending ");
  //RP_Print_Packet(packet);
  if(interface->send(interface->link_handler, interface->can_id_tx, interface->buffer_out, len, timeout) != 0){
    err = RP_ERR_LINK | RP_ERR_TIMEOUT;
    return -1;
  }
...
int RP_Wait_Packet(RP_Interface *interface, uint32_t timeout_ms){
  ...
  while(!interface->received && (interface->get_tick() - start < timeout_ms)){
    uint32_t id;
    uint8_t data[8];
    int count = can_read(&id, data);
    if(count && count != -1 && ((id & 0x7FF) == interface->can_id_rx)){
      RP_Process_Data(interface, data, count);
    }
  }
