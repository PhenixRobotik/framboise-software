...
//RP_MAX_PACKET_SIZE must be <= 254 (see RP_Packet below).
#define RP_MAX_PACKET_SIZE 254
 ...
 *
 * id contains the ID field of the CAN frames for the packet
 */
...
typedef struct RP_Interface_S{
  ...
  uint32_t can_id_rx;
  /**
   * ID used to identify this interface.
   * Bits [11:6] : remote node
   * Bits [5:0]  : local node
   */
}RP_Interface;
